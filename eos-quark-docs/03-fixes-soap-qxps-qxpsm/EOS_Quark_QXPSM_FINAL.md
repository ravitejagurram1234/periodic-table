...
| 2b | Strip `<wsdl:fault name="QException">` from WSDL (Axis 1.x fails to generate the `QException` class → 42 `cannot find symbol`). Regex-removed from portType+binding; runtime faults still arrive as `AxisFault`/caught | ✅ done |
...
| 8 | Add + run `QxpsmProbe` to verify server contract | ✅ done — **L1 + L2 PASSED** (DEV `srvcldvapd001:8090`; server QuarkXPress Server 21.0.2.0; polymorphism confirmed — L2 returned full SERVERINFO). Delete `QxpsmProbe.java` in step 10. |
