...
/* ---------------------------------------------------------------------------
   ...
   Run immediately after applying either QXP_PK_SUIVI compatibility change.
   PASS means exactly two OBJECT rows (PACKAGE and PACKAGE BODY), both VALID,
   with no ERROR or INVALID_DEPENDENT rows. The query is read-only and returns
   at most 20 compiler errors and 20 invalid direct database dependents.
   --------------------------------------------------------------------------- */
SELECT
...
FROM (
    ...
    SELECT
        3,
        'INVALID_DEPENDENT',
        dependent_object.object_type,
        dependent_object.status,
        CAST(NULL AS NUMBER),
        CAST(NULL AS NUMBER),
        dependent_object.object_name
    FROM user_objects dependent_object
    WHERE dependent_object.status = 'INVALID'
      AND EXISTS (
          SELECT 1
          FROM user_dependencies dependency_record
          WHERE dependency_record.name = dependent_object.object_name
            AND dependency_record.type = dependent_object.object_type
            AND dependency_record.referenced_name = 'QXP_PK_SUIVI'
            AND dependency_record.referenced_type = 'PACKAGE'
      )
) compile_evidence
ORDER BY
